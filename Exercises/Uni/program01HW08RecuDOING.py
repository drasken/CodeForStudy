...
def findDivColour(table): #funzione per trovare colore del bordo o di una linea
    for rows in table:
        colour = rows[0]
        check = all([item == colour for item in rows])
        if check:
            return colour
...
def ex1(input_file,  output_file):
    ...
    provaLinea = {x for x in decField[0]}  
    
    # col_div_vert = []
    # for i in decField[-1]:
    #     if i in provaLinea and i != bg_color:
    #         col_div_vert.append(i)
    #         col_div_vert.append(decField[-1].index(i))
            
    provaColore = findDivColour(decField)
    
    return provaLinea,bg_color,provaColore
    pass
