...
;; PUZZLE: The submarine has been making some odd creaking noises, so
...
(defparameter my-data (read-my-input))

(defun transpose-strings (my-input)
  "Given my-input — a list of equal-length strings - return the equivalent
transposed strings."
  (when (null my-input) nil)  ; check for null input
  (loop for i below (length (car my-input))
	collect
	 (loop for j below (length my-input)
			  collect (char (nth j my-input) i))))

(defparameter my-data (transpose-strings my-data))

;; Now get the most common, implement func for each line
;; Use this as reference to get a concatenated string
;; (funcall #'(lambda (x) (coerce x 'string)) (car my-test))
;; or
;; (apply #'map 'list (lambda ...) input)

(defun transpose-correct (my-input)
 "Reunite the list of chars in a list of strings and get the desired transpose"
  (mapcar (lambda (el) (coerce el 'string)) my-input)) 

(defparameter my-data (transpose-correct my-data))

;; Get the most and least common bit
;; (defun get-mcbit (lst)
;;   "From a list of string of 0 and 1 chars get the most and list chars"
;;   (let ((res '()))
;;     (cond ((> (str:count-substring "1" (car lst))
;; 	      (str:count-substring "0" (car lst))) "1")
;; 	  (t "0"))
