...
def checkUfoLand(ufo_mes: tuple[int], matrix:list[list]) -> bool:

    #larghezza W, altezza H e distanza minima D value in tuple
    width, height, padding= ufo_mes[0], ufo_mes[1], ufo_mes[2]
    black = (0,0,0)
    
    check_width = len(matrix[0]) - (width + padding)
    check_height = len(matrix) - (height + padding)
    ...
    for rowNum, row in enumerate(matrix):
        if rowNum < padding:
            continue
        if rowNum > check_height:
            break
        for columnNum, color in enumerate(row):
            if color != black or columnNum > check_width or columnNum < padding:
                continue
            else:
                check_set = set()
                #first check horizontal rectangle
                for row_hor in matrix[rowNum: rowNum + height]:
                    for color_hor in row_hor[columnNum - padding: columnNum + width + padding]:
                        check_set.add(color_hor)
                #now check for vertical rectangle
                for row_vert in matrix[rowNum - padding: rowNum + height + padding]:
                    for color_vert in row_vert[columnNum: columnNum + width]:
                        check_set.add(color_vert)
                
                check_set.discard(black)
                if check_set == set():
                    return True
...
def ex(file_png, file_txt, file_out):
    ...
    for ufo in listUfos:
        ...
        checkUfo = checkUfoLand(ufo, matrix)
...
if __name__ == "__main__":
    ...
    matrix3 = load_png8('images/image3.png')
    
    provaUfo = checkUfoLand((6,4,1), matrix3)
